...
function Clue() {
  ...
  const currentPhase = clueId <= 5 ? 'phase1' : 
                   clueId <= 10 ? 'phase2' : 
                   'phase3';
const currentClue = clues[currentPhase]?.find(clue => clue.id === clueId) || {
  ...
  const { currentClueId } = loadProgress();


 
    // 3. Better last clue detection
const isLastClueOfPhase = 
  ...
  useEffect(() => {
    setVerified(false);
    setShowBackupOption(false); // Add this line
    setBackupAnswer(''); // Also reset the input field
    saveProgress(currentPhase, parseInt(id));
  }, [id]); // Runs whenever the clue ID changes

  const handleVerificationSuccess = () => {
    setVerified(true); // Only update verification state here
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (distance < 2) { // Within ~2000 meters
            handleVerificationSuccess(); // Call on success
            
          } else {
            alert("You're not quite at the right location yet. Keep searching!");
          }
          setIsVerifying(false); // Stop loading when done
          //setVerified(true);
        },
        (error) => {
          console.error("Error getting location:", error);
          setIsVerifying(false); // Stop loading on error too
        }
      );
    } else {
...
return (
   <div style={{
        ...
        zIndex: 1, // Ensures content appears above floating hearts
        }}>
  <AnimatePresence mode="wait">
    <motion.div
      key={id} // Important: use clue ID as key
  ...
  <div style={{
    ...
    background: 'linear-gradient(to right, #fff5f5, #ffecec)' // Romantic gradient background
  }}>
    ...
    {!verified && (
      ...
      {/* Backup input field (only shows when toggled) */}
        {showBackupOption && (
